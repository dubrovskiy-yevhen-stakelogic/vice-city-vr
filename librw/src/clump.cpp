...
Clump::clone(void)
{
	...
	FORLIST(lnk, this->atomics){
		...
		// Badly converted replacement models can carry an atomic that has no
		// frame. Cloning such a clump dereferenced the missing frame every time
		// the model spawned and crashed the game; skip the orphan instead. A
		// failed atomic clone under memory pressure took the same path.
