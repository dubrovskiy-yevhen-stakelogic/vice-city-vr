...
// Consumes the palette and every mip level of a native texture whose raster
// could not be allocated, so the stream stays aligned for the textures that
// follow it in the dictionary.
static void
skipNativeTextureData(Stream *stream, int32 numLevels, int32 pallength)
{
	if(pallength != 0)
		stream->seek(4*pallength);
	for(int32 i = 0; i < numLevels; i++)
		stream->seek(stream->readU32());
}
...
readNativeTexture(Stream *stream)
{
	...
	if(compression){
		...
		// Raster::create returns nil when the driver cannot allocate the
		// resource, which is what happens once high resolution replacement
		// textures exhaust video memory. Dereferencing it here wrote through a
		// null pointer and took the whole game down, so a texture that cannot
		// be allocated is skipped instead: its data is consumed from the
		// stream and the texture is left without a raster.
		if(raster == nil){
			skipNativeTextureData(stream, numLevels, pallength);
			return tex;
		}
	...
	for(int32 i = 0; i < numLevels; i++){
		size = stream->readU32();
		// The stored size comes straight out of the file while the buffer
		// behind it is sized by the engine. A level that claims to be larger
		// than its own dimensions allow would overrun that buffer, so read
		// only what fits and skip the rest.
		int32 capacity = i < raster->getNumLevels() ? getLevelSize(raster, i) : 0;
		if(i < raster->getNumLevels() && capacity > 0){
			data = raster->lock(i, Raster::LOCKWRITE|Raster::LOCKNOFETCH);
			if(data == nil)
				stream->seek(size);
			else{
				uint32 readable = size > (uint32)capacity ? (uint32)capacity : size;
				stream->read8(data, readable);
				raster->unlock(i);
				if(size > readable)
					stream->seek(size - readable);
			}
		}else
			stream->seek(size);
	}
