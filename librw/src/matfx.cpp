...
MatFX::enableEffects(Atomic *atomic)
{
	*PLUGINOFFSET(int32, atomic, matFXGlobals.atomicOffset) = 1;
	ObjPipeline *pipeline = matFXGlobals.pipelines[rw::platform];
#ifdef RW_D3D12
	// D3D12 does not have a dedicated MatFX shader pipeline yet. Leaving the
	// plugin's dummy pipeline attached makes every MatFX-enabled atomic a render
	// no-op; the INT_M MERCED clump then shows only its four non-MatFX wheels.
	// Render the base material through the complete D3D12 world pipeline until
	// environment-map support is implemented, matching the backend's existing
	// compatibility policy for Neo/custom material pipelines.
	if(rw::platform == PLATFORM_D3D12 &&
	   pipeline == matFXGlobals.dummypipe && engine &&
	   engine->driver[PLATFORM_D3D12] &&
	   engine->driver[PLATFORM_D3D12]->defaultPipeline)
		pipeline = engine->driver[PLATFORM_D3D12]->defaultPipeline;
#endif
